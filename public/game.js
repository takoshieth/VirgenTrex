...
(function() {
  ...
  // Pointer events first (modern browsers including iOS Safari/Chrome)
  if (window.PointerEvent) {
    canvas.addEventListener('pointerdown', pressStart, { passive: false });
    canvas.addEventListener('pointerup', pressEnd, { passive: true });
    canvas.addEventListener('pointercancel', pressEnd, { passive: true });
  }
  // Fallbacks
  canvas.addEventListener('touchstart', pressStart, { passive: false });
  canvas.addEventListener('touchend', pressEnd, { passive: true });
  canvas.addEventListener('mousedown', pressStart, { passive: false });
  canvas.addEventListener('mouseup', pressEnd, { passive: true });
...
// Attach start handler robustly (iOS Safari quirks)
['click','touchend','pointerup','mousedown','mouseup'].forEach((ev)=>{
    try { playBtn.addEventListener(ev, startGameFromUI, {passive:false, capture:true}); } catch (_) { playBtn.addEventListener(ev, startGameFromUI); }
  });
